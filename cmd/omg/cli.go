...
func Execute() {
	...
	convDenom := flag.String("cvd", "", fmt.Sprintf("Convert (%s) to token (%s) amount", cfg.Denom, cfg.Token))
	convToken := flag.String("cvt", "", fmt.Sprintf("Convert (%s) to denom (%s) amount", cfg.Token, cfg.Denom))
